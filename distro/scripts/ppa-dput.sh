...
#
# Example:
#   ./upload-ppa.sh ../matugen_2.4.1ppa1_source.changes danklinux
#   ./upload-ppa.sh ../quickshell-git_0.3.0+git1024.abc123ppa1_source.changes danklinux
...
if [ $# -lt 2 ]; then
    ...
    echo
    echo "Arguments:"
    echo "  changes-file : Path to .changes file (e.g., ../matugen_2.4.1ppa1_source.changes)"
    echo "  ppa-name     : PPA to upload to (danklinux)"
    echo
    echo "Examples:"
    echo "  $0 ../matugen_2.4.1ppa1_source.changes danklinux"
    echo "  $0 ../quickshell-git_0.3.0+git1024.abc123ppa1_source.changes danklinux"
    exit 1
fi
...
# Validate PPA name
if [ "$PPA_NAME" != "danklinux" ]; then
    error "PPA name must be 'danklinux'"
    exit 1
fi
